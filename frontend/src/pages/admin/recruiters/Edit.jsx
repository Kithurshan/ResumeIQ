...
const Edit = () => {
  ...
  useEffect(() => {
    const fetchRecruiter = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/recruiters/profile/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        const response = await fetch(`http://localhost:5000/api/recruiters/update/${id}`, {
          method: 'PUT',
          headers: {
    ...
    if (file) {
      ...
      try {
        ...
        if (uploadResult.success && uploadResult.imageUrl) {
          ...
          await fetch(`http://localhost:5000/api/recruiters/update-profile-image/${id}?image_url=${encodeURIComponent(uploadResult.imageUrl)}`, {
            method: 'PUT'
          });
        }
      } catch (err) {
