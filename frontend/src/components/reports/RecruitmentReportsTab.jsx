...
import { FiSearch, FiEye, FiBriefcase } from 'react-icons/fi';

const RecruitmentReportsTab = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        console.error("Failed to fetch recruitment reports:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const JOBS = [...new Set(reports.map(r => r.job))];
  const RECRUITERS = [...new Set(reports.map(r => r.recruiter).filter(Boolean))];

  let filteredReports = reports.filter(rep => {
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      if (!rep.job.toLowerCase().includes(q) &&
        (!rep.recruiter || !rep.recruiter.toLowerCase().includes(q))) return false;
    }
    if (jobFilter !== 'All' && rep.job !== jobFilter) return false;
    if (recruiterFilter !== 'All' && rep.recruiter !== recruiterFilter) return false;
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-[#111111] text-gray-400 font-medium border-b border-gray-800">
              <tr>
                ...
                <th className="px-6 py-4 text-center">Waitlisted</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {paginatedReports.map((rep) => (
                <tr key={rep.id} className="hover:bg-[#252525] transition-colors text-gray-300">
                  ...
                  <td className="px-6 py-4 text-gray-400">{rep.recruiter || 'Unassigned'}</td>
                  <td className="px-6 py-4 text-center font-medium">{rep.uploaded || 0}</td>
                  <td className="px-6 py-4 text-center font-medium text-emerald-500">{rep.shortlisted || 0}</td>
                  <td className="px-6 py-4 text-center font-medium text-amber-500">{rep.waitlisted || 0}</td>
                  <td className="px-6 py-4 text-center font-medium text-red-500">{rep.rejected || 0}</td>
                  <td className="px-6 py-4">
                    <div className="flex justify-center">
                      <button
                        onClick={() => navigate(`/admin/reports/recruitment/${rep.id}`)}
...
export default RecruitmentReportsTab;
